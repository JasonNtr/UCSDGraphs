...
public class GraphAdjList extends Graph {
	...
	/** 
	 * Implement the abstract method for finding all 
	 * vertices reachable List<Integer> dis= new ArrayList <Integer>();
		 
		 for(int i=0;i<getNeighbors(v).size();i++) {
			 List<Integer> nei= new ArrayList <Integer>(getNeighbors(i));
			 dis.addAll(nei);
		 }
		
		 
		 for(int i=0;i<getNeighbors(v).size();i++) {
			 List<Integer> nei= new ArrayList <Integer>(getInNeighbors(i));
			 dis.addAll(nei);
		 }
		 for(int i=0;i<getInNeighbors(v).size();i++) {
			 List<Integer> nei= new ArrayList <Integer>(getNeighbors(i));
			 dis.addAll(nei);
		 }
		 for(int i=0;i<getInNeighbors(v).size();i++) {
